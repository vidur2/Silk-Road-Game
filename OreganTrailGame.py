...
def newDay(statsDictionary, difficulty, weight, inventory):
    ...
    statsDictionary['milesPerTurn'] = (300)*(0.83333333333333333333)*(statsDictionary['food'] + statsDictionary['water'] + statsDictionary['durability'])/weightOfThings(inventory, weight)
    speedRaw = statsDictionary['milesPerTurn']/300
    estTurns = round(1/speedRaw)
    statsDictionary['estimatedTurnsToCity'] = estTurns 
...
def nextAction(inventory, value, stage, hasHit, statsDictionary, difficulty, chineseMarkets, weekCounter, weight):
    ...
    if action == 'map':
      ...
      print(statsDictionary)
        ...
        subVar = 0
        counterTurn = 1
        if counterTurn < statsDictionary['estimatedTurnsToCity']:
            print("After a day of travel....")
            statsDictionary['estimatedTurnsToCity'] = statsDictionary['estimatedTurnsToCity'] - 1
        elif counterTurn == statsDictionary['estimatedTurnsToCity']: 
          stage = stage + 1
        
        

        isAlive = newDay(statsDictionary=statsDictionary, difficulty=difficulty, weight=weight, inventory=inventory)
        speed = statsDictionary['milesPerTurn']
